...
function BookInputForm({ member, month, year, onSubmit, submitting }: BookInputFormProps) {
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="mt-4 space-y-3">
        ...
        <button
          ...
          className="w-full rounded-lg bg-amber-500 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-amber-600 disabled:cursor-not-allowed disabled:opacity-50"
        >
...
function BookDisplay({ book, light = false }: BookDisplayProps) {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      ...
      {hasCurrentBook ? (
        ...
        <div className="rounded-2xl border-2 border-dashed border-amber-300 bg-amber-50 p-6 dark:border-amber-800 dark:bg-amber-950/30">
      ...
      {hasNextBook ? (
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <BookDisplay book={nextBook} light />
        </div>
      ) : (
